...
int fxos8700_read(struct device *bus, u8_t reg,
			    u8_t *data, u16_t len)
{
	struct spi_config *spi_cfg = &fxos8700_spi_conf;
	u8_t buffer_tx[2] = { reg & ~FXOS8700_SPI_WRITE, 0 };
...
int fxos8700_write(struct device *bus, u8_t reg,
			     u8_t *data, u16_t len)
{
	struct spi_config *spi_cfg = &fxos8700_spi_conf;
	u8_t buffer_tx[1] = { reg | FXOS8700_SPI_WRITE };
