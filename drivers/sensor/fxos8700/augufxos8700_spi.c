...
#ifdef DT_NXP_AUGUFXOS8700_BUS_SPI

#define AUGU_FXOS8700_SPI_WRITE (1 << 7)

LOG_MODULE_DECLARE(AUGU_FXOS8700, CONFIG_SENSOR_LOG_LEVEL);

static struct spi_config fxos8700_spi_conf = {
	.frequency = DT_INST_0_NXP_AUGUFXOS8700_SPI_MAX_FREQUENCY,
	.operation = (SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB | SPI_WORD_SET(8) |
		      SPI_LINES_SINGLE),
	.slave = DT_INST_0_NXP_AUGUFXOS8700_BASE_ADDRESS,
	.cs = NULL,
};
...
int fxos8700_read(struct device *bus, u8_t reg, u8_t *data, u16_t len)
{
	...
	u8_t buffer_tx[2] = { reg & ~AUGU_FXOS8700_SPI_WRITE, (reg & 0x80) >> 7 };
...
int fxos8700_spi_cs_gpios_init(struct fxos8700_data *data)
{
	...
	data->cs_ctrl.gpio_dev =
		device_get_binding(DT_INST_0_NXP_AUGUFXOS8700_CS_GPIOS_CONTROLLER);
	if (!data->cs_ctrl.gpio_dev) {
		LOG_ERR("Unable to get GPIO SPI CS device");
		return -ENODEV;
	}

	data->cs_ctrl.gpio_pin = DT_INST_0_NXP_AUGUFXOS8700_CS_GPIOS_PIN;
	data->cs_ctrl.delay = 0U;

	fxos8700_spi_conf.cs = &data->cs_ctrl;

	LOG_DBG("SPI GPIO CS configured on %s:%u",
		DT_INST_0_NXP_AUGUFXOS8700_CS_GPIOS_CONTROLLER,
		DT_INST_0_NXP_AUGUFXOS8700_CS_GPIOS_PIN);
...
#endif /* DT_INST_0_NXP_AUGUFXOS8700_CS_GPIOS_CONTROLLER */

#endif /* DT_NXP_AUGUFXOS8700_BUS_SPI */